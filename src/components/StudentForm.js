...
function StudentForm({ student, type }) {
  ...
  const onFinish = async (values) => {
    try {
      ...
      let response = null;
      if (type === "edit") {
       response = await axios.post(`https://crm-backend-yrar.onrender.com/student/update-student/${student.rollNo}`, values, {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
          },
        });
      } else {
       response = await axios.post("https://crm-backend-yrar.onrender.com/student/add-student", values, {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
          },
        });
